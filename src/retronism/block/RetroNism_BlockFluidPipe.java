...
public class Retronism_BlockFluidPipe extends BlockContainer {
	...
	public boolean canConnectTo(IBlockAccess world, int x, int y, int z) {
		...
		return Retronism_PortRegistry.isPortOfType(x, y, z, Retronism_PortRegistry.PORT_TYPE_FLUID);
	}
	...
	public boolean canConnectToSide(IBlockAccess world, int myX, int myY, int myZ, int side) {
		...
		if (myTe instanceof Retronism_ISideConfigurable) {
			int mode = Retronism_SideConfig.get(((Retronism_ISideConfigurable) myTe).getSideConfig(), side, Retronism_SideConfig.TYPE_FLUID);
		...
		if (nTe instanceof Retronism_ISideConfigurable) {
			...
			int mode = Retronism_SideConfig.get(((Retronism_ISideConfigurable) nTe).getSideConfig(), opp, Retronism_SideConfig.TYPE_FLUID);
			if (mode == Retronism_SideConfig.MODE_NONE) return false;
		}
		return true;
	}
