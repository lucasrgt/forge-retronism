...
public class Retronism_BlockAlloyBlenderController extends BlockContainer {
    public Retronism_BlockAlloyBlenderController(int id, int tex) {
        super(id, tex, Material.iron);
        setHardness(3.5F);
        setResistance(5.0F);
        setStepSound(Block.soundMetalFootstep);
        setBlockName("alloyblenderController");
    }

    @Override
    protected TileEntity getBlockEntity() {
        return new Retronism_TileAlloyBlender();
    }

    @Override
    public boolean blockActivated(World world, int x, int y, int z, EntityPlayer player) {
        if (world.multiplayerWorld) return true;
        Retronism_TileAlloyBlender tile = (Retronism_TileAlloyBlender) world.getBlockTileEntity(x, y, z);
        ...
        ModLoader.OpenGUI(player, new Retronism_GuiAlloyBlender(player.inventory, tile));
        return true;
    }
